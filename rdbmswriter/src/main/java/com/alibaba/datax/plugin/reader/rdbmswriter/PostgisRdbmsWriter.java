                ...
                switch (columnSqltype) {
                    ...
                    case Types.JAVA_OBJECT:
                        if (null == column.getRawData()) {
                            ...
                            if (rawData instanceof BytesColumn) {
                                BytesColumn bytesColumn = (BytesColumn) rawData;
                                Object parsed = JSON.parse(bytesColumn.asBytes());
                                if (parsed instanceof PostgisWrapper) {
                                    PostgisWrapper postgisWrapper = (PostgisWrapper) parsed;
                                    Object pgRawData = postgisWrapper.getRawData();
                                    preparedStatement.setObject(columnIndex + 1, pgRawData);
                                } else {
                                    LOG.error("PostgisWriter write column {} with type {} is not PostgisWrapper, please check your database data.",
